...
SearchSpaceQuery::SearchSpaceQuery(const Graph &graph, const OverlayGraph &overlayGraph, const std::vector<Metric> &metrics, PathUnpacker &pathUnpacker) : graph(graph), overlayGraph(overlayGraph), metrics(metrics), pathUnpacker(pathUnpacker)
{
...
CRP::SearchSpaceResult SearchSpaceQuery::vertexQuery(index sourceVertexId, index targetVertexId, index metricId)
{
...
CRP::SearchSpaceResult SearchSpaceQuery::edgeQuery(index sourceEdgeId, index targetEdgeId, index metricId)
{
  currentRound++;
  ...
  while (forwardGraphPQ.size() + forwardOverlayGraphPQ.size() > 0 && backwardGraphPQ.size() + backwardOverlayGraphPQ.size() > 0)
  {
    if (shortestPath < std::min(forwardGraphPQ.peekKey(), forwardOverlayGraphPQ.peekKey()) + std::min(backwardGraphPQ.peekKey(), backwardOverlayGraphPQ.peekKey()))
      break;

    weight minGraph = std::min(forwardGraphPQ.peekKey(), backwardGraphPQ.peekKey());
    weight minOverlay = std::min(forwardOverlayGraphPQ.peekKey(), backwardOverlayGraphPQ.peekKey());

    if (minGraph < minOverlay)
    { // graph vertex
      if (forwardGraphPQ.peekKey() < backwardGraphPQ.peekKey())
      { // search forward
        ...
        for (index j = 0, entryId = graph.getEntryOffset(u) - offset; entryId < graph.getEntryOffset(u) + uInDeg - offset; ++entryId, ++j)
        {
          ...
          { // only if we do not have a valid distance label already
        ...
        graph.forOutEdgesOf(u, entryPoint, [&](const ForwardEdge &e, index exitPoint, Graph::TURN_TYPE type) {
          ...
          visitedVertices.insert(std::make_pair(v, vQueryLevel));
          ...
          { // graph
            ...
            if (forwardInfo[vId].round < currentRound && newDist > forwardInfo[vId].dist)
              return; // we haven't seen vId yet and we cannot improve anything from this entryPoint
          ...
          { // v is in another cell on another level
            ...
            if (forwardInfo[vId].round < currentRound || newDist < forwardInfo[vId].dist)
            {
              ...
              forwardInfo[vId].round = currentRound;
              forwardInfo[vId].parent = {u, uId};
              if (backwardInfo[vId].round == currentRound && forwardInfo[vId].dist + backwardInfo[vId].dist < shortestPath)
              {
                shortestPath = forwardInfo[vId].dist + backwardInfo[vId].dist;
                forwardMid = {v, vId};
                backwardMid = {v, vId};
              }
            }
        ...
        graph.forInEdgesOf(u, exitPoint, [&](const BackwardEdge &e, index entryPoint, Graph::TURN_TYPE type) {
          ...
          { // v is in another cell on another level
            ...
            if (backwardInfo[vId].round < currentRound || backwardInfo[uId].dist + edgeWeight < backwardInfo[vId].dist)
            {
              ...
              backwardInfo[vId].round = currentRound;
              backwardInfo[vId].parent = {u, uId};
              if (forwardInfo[vId].round == currentRound && forwardInfo[vId].dist + backwardInfo[vId].dist < shortestPath)
              {
                shortestPath = forwardInfo[vId].dist + backwardInfo[vId].dist;
                forwardMid = {v, vId};
                backwardMid = {v, vId};
    ...
    { // overlay graph
      if (forwardOverlayGraphPQ.peekKey() < backwardOverlayGraphPQ.peekKey())
      { // search forward
        ...
        overlayGraph.forOutNeighborsOf(u, uQueryLevel, [&](index v, index wOffset) {
          visitedVertices.insert(std::make_pair(v, uQueryLevel));
          ...
          if (forwardInfo[vId].round < currentRound || newDist < forwardInfo[vId].dist)
          {
            ...
            {
              shortestPath = forwardInfo[vId].dist + backwardInfo[vId].dist;
              forwardMid = {vVertex.originalVertex, vId};
              backwardMid = {vVertex.originalVertex, vId};
            }
            ...
            { // we are back on the graph
              ...
              if (forwardInfo[originalWId].round < currentRound && newDist > forwardInfo[originalWId].dist)
                return; // we haven't seen originalWId yet and we cannot improve anything from this entryPoint
            ...
            else
            {
              ...
              if (forwardInfo[wId].round < currentRound || newDist < forwardInfo[wId].dist)
              {
                ...
                if (backwardInfo[wId].round == currentRound && forwardInfo[wId].dist + backwardInfo[wId].dist < shortestPath)
                {
                  shortestPath = forwardInfo[wId].dist + backwardInfo[wId].dist;
          ...
          if (backwardInfo[vId].round < currentRound || newDist < backwardInfo[vId].dist)
          {
            ...
            { // we are back on the graph
              ...
              if (backwardInfo[originalWId].round < currentRound && newDist > backwardInfo[originalWId].dist)
                return; // we haven't seen originalWId yet and we cannot improve anything from this exitPoint
            ...
            else
            {
              ...
              if (backwardInfo[wId].round < currentRound || newDist < backwardInfo[wId].dist)
              {
                ...
                if (forwardInfo[wId].round == currentRound && forwardInfo[wId].dist + backwardInfo[wId].dist < shortestPath)
                {
                  shortestPath = forwardInfo[wId].dist + backwardInfo[wId].dist;
                  forwardMid = {wVertex.originalVertex, wId};
                  backwardMid = {wVertex.originalVertex, wId};
  ...
  {
    return {visitedVertices, {}, inf_weight };
  }
  ...
  return {visitedVertices, unpackedPath, shortestPath};
}
