...
export async function generatePairs (count: number): Promise<Array<{src: string, dest: string}>> {
  ...
  const edges: {
    ...
    bottomLeft: string[]
    bottomRight: string[]
  } = JSON.parse(await fs.readFile(resolvePath(['experiments', 'traffic', 'edges.json']), 'utf-8'))
